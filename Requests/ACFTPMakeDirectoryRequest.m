...
-(void)start {
	...
	self.directoryURL = [NSString stringWithFormat:@"%@%@%@", strUrl, (([strUrl hasSuffix:@"/"]) ? @"" : @"/"), self.name];
    ...
    switch (eventCode) {
        ...
        case NSStreamEventErrorOccurred: {
            ...
            if (err.domain == kCFStreamErrorDomainFTP) {
                [self didFail:[NSString stringWithFormat:@"FTP error %d", (int)err.error]];
            } else {
                [self didFail:@"Stream open error"];
            }
        } break;
...
-(void)didFail:(NSString*)reason {
	[self stop];
	NSError* error = [NSError errorWithDomain:@"com.mobilu.onsong" code:502 userInfo:[NSDictionary dictionaryWithObject:reason forKey:@"reason"]];
	if(self.delegate != nil && [(NSObject*)self.delegate respondsToSelector:@selector(request:didFailWithError:)]) {
		[(NSObject*)self performSelector:@selector(request:didFailWithError:) withObject:self withObject:error];
	} else {
